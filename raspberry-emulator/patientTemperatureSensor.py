...
class Temperature_sensor():
    ...
    def emulate(self,ID_P):
        ...
        if ID_P==1:
            ...
            self.__baseMessage["e"][1]["v"]=round(random.uniform(low=36.45,high=36.55),2)
        elif ID_P==2:
            # Paziente sano con batteria scarica
            self.__baseMessage["e"][0]["v"]=round(random.uniform(low=2.77,high=2.79),2)
            self.__baseMessage["e"][1]["v"]=round(random.uniform(low=36.45,high=36.55),2)
        elif ID_P==3:
            ...
            self.__baseMessage["e"][0]["v"]=round(random.uniform(low=2.81,high=3.0),2)
            self.__baseMessage["e"][1]["v"]=round(random.uniform(low=34,high=35),2)
        elif ID_P==4:
            # Paziente con febbre e con batteria carica
            self.__baseMessage["e"][0]["v"]=round(random.uniform(low=2.81,high=3.0),2)
            self.__baseMessage["e"][1]["v"]=round(random.uniform(low=37.1,high=37.5),2)
        else:
            # Paziente con temperatura bassa e con batteria carica
            self.__baseMessage["e"][0]["v"]=round(random.uniform(low=2.81,high=3.0),2)
            self.__baseMessage["e"][1]["v"]=round(random.uniform(low=35.1,high=35.9),2)
