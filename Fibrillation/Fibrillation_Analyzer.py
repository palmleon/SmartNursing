...
class Fibrillation_Monitor():
    def __init__(self,messagesdict,voltageTheresold,fibrillationState,attendabilityTheresold):
        ...
        self.__voltageTheresold = voltageTheresold
        self.__fibrillationState = fibrillationState
    ...
    def fibrillation(self,ID_P,PI,pulse,battery):
        ...
        if battery<self.__voltageTheresold :# Per informare che il pulsossimetro si sta scaricando (es. suppongo sia scarico a 2.5V)
        ...
        if len(pulse)<self.__fibrillationState: # Controllo aggiunto in quanto, se vengono rimosse troppe letture, non ha senso valutare la deviazione standard
            alarm=self.__alarm_place[0]+ID_P+self.__alarm_place[1]
            #return f"ATTENZIONE, il paziente {ID_P} ha il pulsossimetro mal posizionato o l'ha rimosso"
            return alarm
        ...
        if pulse>self.__fibrillationState: # Soglia di deviazione standard che definische lo stato di fibrillazione
